...
![图片描述](https://img.mukewang.com/wiki/662b45f9096fa37a21541022.jpg)
...
<font color="#fd6f01">**递归互斥信号量是互斥信号量的一种特殊形式，它允许同一个线程多次获取同一个锁**</font>。这在递归函数或可重入代码块中非常有用，因为它允许同一个线程在已经持有锁的情况下再次获取锁，而不会导致死锁。
...
优先级翻转（Priority Inversion）是一种在实时操作系统中可能发生的调度问题，它发生在两个或多个进程或线程具有不同的优先级，并且它们之间的<font color="#fd6f01">**资源请求和释放顺序**</font>可能<font color=red>**导致低优先级任务比高优先级任务更早完成**</font>的情况。

<font color=green>**这种现象是一个较低优先级的任务阻塞了一个较高优先级任务的执行，从而导致高优先级任务无法及时完成。**</font>

![图片描述](https://img.mukewang.com/wiki/662b46250971e0ce22481414.jpg)
...
2.注意不能使用osDelay(会从就绪列表迁移到延时列表，阻塞，让出CPU执行权)
...
![图片描述](https://img.mukewang.com/wiki/662b463d097e7f3b26000652.jpg)
...
```c
xEventGroupSync 函数是 FreeRTOS 中的一个事件组操作函数，它结合了事件位的设置和等待操作。
这个函数允许任务设置一组事件位，并等待另一组事件位被设置或者到达某个时间点。
...
- 使用队列、信号量、事件标志组时都需创建一个结构体，通过中间的结构体进行间接通信！
  ![图片描述](https://img.mukewang.com/wiki/662b466f09425d4010890494.jpg)

> 使用队列/信号量/事件标志组时发送数据，**就是把发送数据放在队列/信号量/事件标志组的结构体中**，接收也是从队列/信号量/事件标志组的结构体中读出来。

- 使用任务通知时，**任务结构体TCB中就包含了内部对象，可以直接接收别人发过来的"通知"**

![图片描述](https://img.mukewang.com/wiki/662b46880909709d11750418.jpg)
...
![图片描述](https://img.mukewang.com/wiki/662b469a097a5c3211110430.jpg)
...
其中任务通知状态共有三种值:
![图片描述](https://img.mukewang.com/wiki/662b46af09324e1812800186.jpg)
...
![图片描述](https://img.mukewang.com/wiki/662b46d309cc4a4f07630190.jpg)
...
​     ISR没有任务结构体，所以无法给ISR发送数据。<font color="#fd6f01">**但是ISR可以使用任务通知的功能，发数据给任务**。</font>

   （发送可以不能接收）

- **无法广播给多个任务**

​     任务通知只能是被指定的一个任务接受并处理

- **无法缓存多个数据**

​     任务通知是通过更新任务通知值来发送数据的，**任务结构体中只有一个任务通知值，只能一个数据**。

  （队列的话，只要有m个队列项就保存m个数据）

- **发送受阻不支持阻塞**

​      发送方无法进入阻塞状态等待（队列已满，可以阻塞）
