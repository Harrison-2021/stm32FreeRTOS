...
- 宏
  ...
  - 宏以定义宏的文件名或文件名缩写为前缀。前缀为小写。例如，在 FreeRTOSConfig.h 中定义 **config**USE_PREEMPTION。
...
3.`st`设计时，在初始化时，`MX_FREERTOS_Ini`t中会调用默认执行任务，一般为死循环

4.FreeRTOS系统会在初始化时在在`pendsv`中断执行空闲任务
...
```c
osThreadId_t defaultTaskHandle; // 新任务分配的内存地址
